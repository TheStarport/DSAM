...
///
/// Here lies the old version of FLDataFile to be rewritten.
/// Danger, High Voltage!
/// 

namespace FLDataFile.Obsolete
{
    public class FLDataFile
    {
        ...
        public class Setting
        {
            ...
            public string UniStr(int index) { return DecodeUnicodeHex(Convert.ToString(values[index])); }
        ...
        public class Section
        {
            ...
            public Setting GetSetting(string settingName)
            {
                ...
                throw new Exception("setting " + settingName + " not found in section " + sectionName);
            }
        ...
        {
            int p = 4;
            int version = BitConverter.ToInt32(buf, p); p += 4;
            int strTableOffset = BitConverter.ToInt32(buf, p); p += 4;
            ...
            while (p < buf.Length && p < strTableOffset)
            {
                ...
                while (sectionNumEntries-- > 0)
                {
                    ...
                    {
                        int valueType = buf[p++];
                        int value = BitConverter.ToInt32(buf, p); p += 4;
                        switch (valueType)
                        {
                            case 1: // Integer
        ...
        public FLDataFile.Setting GetSetting(string sectionName, string settingName)
        {
            ...
            throw new Exception("section " + sectionName + " not found in " + Path.GetFileName(filePath));
        }
